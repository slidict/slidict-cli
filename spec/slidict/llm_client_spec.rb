...
RSpec.describe Slidict::LLMClient do
  ...
  describe "#verify_connection!" do
    ...
    it "raises an Error when the endpoint responds with a failure" do
      ...
      expect { client.verify_connection! }.to raise_error(Slidict::LLMClient::Error, /400/)
    end
    ...
    it "raises an Error when the connection is refused" do
      ...
      expect { client.verify_connection! }.to raise_error(Slidict::LLMClient::Error)
    end
  end
  ...
  describe "#generate_slides" do
    ...
    it "raises an Error when the HTTP response is not successful" do
      ...
      expect { client.generate_slides(deck) }.to raise_error(Slidict::LLMClient::Error, /400/)
    end
    ...
    it "raises an Error when the model response is not valid JSON" do
      ...
      expect { client.generate_slides(deck) }.to raise_error(Slidict::LLMClient::Error, /no JSON array found/)
    end
    ...
    it "raises an Error when the network request fails" do
      ...
      expect { client.generate_slides(deck) }.to raise_error(Slidict::LLMClient::Error, /connection refused/)
    end
  end
end
